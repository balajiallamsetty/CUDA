...
export default function StudentDashboardPage() {
  ...
  let nowBody = 'Submit a tracked service request when you are ready. A project workspace appears only after CUDA Solutions accepts a request.';
  if (pending > 0 && active === 0) {
    nowTitle = 'Your request is under review';
    nowBody = 'CUDA Solutions is reviewing your request. Your project has not started yet — it will appear under Projects after acceptance.';
  } else if (active > 0) {
    nowTitle = 'You have an active project';
    nowBody = 'Open Projects to see progress, documents, and messages with the CUDA Solutions team.';
  }

  return (
    <div>
      <PageMeta title="Dashboard" description="Your CUDA Solutions Project Assistance workspace." path="/dashboard" />
      <p className="eyebrow">Welcome</p>
      <h1 className="!text-3xl">Hi {user?.name?.split(' ')[0] || 'there'}</h1>
      <p className="lead mb-6">Here is what is happening with your work at CUDA Solutions.</p>
          ...
          <div className="mb-8 grid gap-4 sm:grid-cols-2">
            <article className="rounded-2xl border border-line-soft bg-white p-5 shadow-soft">
              ...
              <p className="mt-1 text-sm text-muted">
                Service requests you sent to CUDA Solutions. Status here is intake status — not development progress.
              </p>
            ...
            <article className="rounded-2xl border border-line-soft bg-white p-5 shadow-soft">
              ...
              <p className="mt-1 text-sm text-muted">
                Requests CUDA Solutions has accepted and started. Documents and messages live here after a project begins.
              </p>
            ...
            <div className="rounded-2xl border border-line-soft bg-white p-6 shadow-soft">
              ...
              {(data.recentProjects || []).length === 0 ? (
                <p className="text-sm text-muted">
                  No projects yet. A project appears here after CUDA Solutions accepts one of your service requests.
                </p>
              ) : (
